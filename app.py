...
def analysis(filename):
    ...
    plt.figure(figsize=(10, 6))
    sns.countplot(data=df, x='estado', order=df['marca'].value_counts().index)
    plt.xticks(rotation=45)
    plt.title('Distribuição por Marca')
    bar_chart_path = os.path.join('static', 'bar_chart.png')
    plt.savefig(bar_chart_path)
    ...
    fig = px.scatter_mapbox(df, lat="latitude", lon="longitude", hover_name="modelo", color="preco", zoom=4, height=500)
